    ...
    */
    ...
    getModeLabel() {
        ...
        switch (this.props.mode) {
            case 'create':
                label = labelProvider.getLabel('Create')
                break
            case 'fuse':
                label = labelProvider.getLabel('Fuse')
                break
            case 'truncate':
                label = labelProvider.getLabel('Truncate')
                break
            case 'expand':
                label = labelProvider.getLabel('Expand')
                break
            case 'delete':
                label = labelProvider.getLabel('Delete')
                break
        }
    ...
    getTitle() {
        ...
        if (this.props.disabled) {
            return labelProvider.getLabel('Disabled')
        }

        // Used only by annotation tool so far
        if (this.props.mode) {
            title = [this.getModeLabel(), title].join(' ')
        }
