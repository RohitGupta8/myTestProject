...
class DragManager extends EventEmitter {
    ...
    */
    _getData(e) {
        ...
        const hasFiles = [...e.dataTransfer.items].some((item) => item.kind === 'file' && item.type !== '')

        if (dataTransfer) {
            return {
                files: hasFiles ? [...dataTransfer.files] : [],
...
class MoveBlockNode extends DragAndDropHandler {
    ...
    match(dragState) {
        const {insertPos} = dragState.dropParams

        // - sourceSeletion must be a NodeSelection
        return !dragState.external && dragState.nodeDrag && dragState.dropType === 'place' && insertPos >= 0
    }
    ...
    drop(tx, dragState) {
        ...
        /**
         * Select dragged node
         * Copy current selection (dragged node)
         * Move selection to target node (if target is after last node, insertMode will be "last")
         */
        ...
        if (!sourceSelection) {
            throw new Error('Invalid dragState, missing sourceSelection')
        }

        let {insertPos} = dropParams
        tx.setSelection(sourceSelection)
        ...
        const nodePos = container.getPosition(nodeId, 'strict')
        ...
        const nodeCopy = tx.get(nodeId)

        if (targetNode === nodeId) {
            // Node is dragged next to itself, just cancel early
            return
        }
