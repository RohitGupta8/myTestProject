...
import markersHelpers from '../model/markersHelpers'
...
class Editing {
    ...
    deleteNodeSelection(tx, sel, direction) {
        ...
        if (sel.isFull() || sel.isBefore() && direction === 'right' || sel.isAfter() && direction === 'left') {
            ...
            // replace the node with default text node if there are no more nodes left
            if (container.nodes.length === 0) {
                ...
                const closestNode = container.getNodeAt(nodePos - 1 < 0 ? nodePos : nodePos - 1)

                if (closestNode.isText()) {
                    tx.setSelection({
                        type: 'property',
                        path: closestNode.getTextPath(),
                        startOffset: closestNode.getLength()
                    })
                }
                else {
                    tx.setSelection({
    ...
    deleteContainerSelection(tx, sel) {
        ...
        if (startPos === endPos) {
            ...
            if (node.isText()) {
                ...
                // If selection is only in list we want to preserve the first empty list item in selection.
                documentHelpers.deleteListRange(tx, node, start, end, {preserveEmptyListItem: true})

                const item = tx.get(start.path[0])
                tx.setSelection({
                    type: 'property',
        ...
        if (lastNode.type === 'list-item') {
            // If list is fully selected delete it, otherwise delete list range.
            if (isEntirelySelected(tx, lastNode.parent, null, end)) {
                tx.update([container.id, 'nodes'], {type: 'delete', pos: endPos})
                documentHelpers.deleteNode(tx, lastNode.parent)
            } else {
                documentHelpers.deleteListRange(tx, lastNode.parent, null, end)
            }
        }
        else if (lastEntirelySelected) {
            tx.update([container.id, 'nodes'], {type: 'delete', pos: endPos})
        ...
        if (firstNode.type === 'list-item') {
            documentHelpers.deleteListRange(tx, firstNode.parent, start, null, {preserveEmptyListItem: true})
        }
        ...
        if (firstNode.type === 'list-item' && start.offset === 0) {
    ...
    insertBlockNode(tx, nodeData) {
        ...
        if (sel.isNodeSelection()) {
            ...
            // insert last
            if (sel.mode === 'last') {
                tx.update(container.getContentPath(), {type: 'insert', pos: container.nodes.length, value: blockNode.id})
            }
            // insert before
            ...
            if (!sel.containerId) {
                throw new Error('insertBlockNode can only be used within a container.')
            }
            ...
            if (node.isList() || (node.parent && node.parent.isList())) {
                ...
                if (sel.mode === 'last') {
                    nodePos = container.nodes.length
                    tx.update(container.getContentPath(), {type: 'insert', pos: nodePos, value: blockNode.id})
                    setCursor(tx, blockNode, container.id, 'before')
                }
                else if (sel.start.offset === 0) {
                    if (text.length === 0) {
                        tx.update(container.getContentPath(), {type: 'delete', pos: nodePos, value: node.id})
                    }
                    tx.update(container.getContentPath(), {type: 'insert', pos: nodePos, value: blockNode.id})
                    setCursor(tx, blockNode, container.id)
                }
                // insert after
     ...
     *
     * @param {object} tx
     * @param {object} data object with `selection`, `containerId` and `data` with new node data
     *
    ...
    toggleList(tx, params) {
        ...
        if (sel.isPropertySelection()) {
            ...
            if (node.isText()) {
                ...
                markersHelpers.transferMarkers(tx, node.getTextPath(), 0, newItem.getTextPath(), 0)
                ...
                markersHelpers.transferMarkers(tx, item.getTextPath(), 0, newTextNode.getTextPath(), 0)
            ...
            // Find the position to add the new list
            let foundFirstNodePos = false
            let nodePos = 0

            nodeIds.forEach((nodeId) => {
                const node = tx.get(nodeId).getRoot()

                if (node.isList()) {
                    if (!foundFirstNodePos) {
                        nodePos = container.getPosition(node.id, 'strict')
                        foundFirstNodePos = true
                    }
                    ...
                    items.forEach(item => {
                        ...
                        const newItem = tx.create({
                            type: 'list-item',
                            content: text,
                        })
                        annotationHelpers.transferAnnotations(tx, item.getTextPath(), 0, newItem.getTextPath(), 0)
                        markersHelpers.transferMarkers(tx, item.getTextPath(), 0, newItem.getTextPath(), 0)

                        newListItems.push(newItem.id)
                    })
                    ...
                    const innerNodePos = container.getPosition(node.id, 'strict')
                    const newItem = tx.create({
                        type: 'list-item',
                        content: text,
                    })
                    ...
                    annotationHelpers.transferAnnotations(tx, node.getTextPath(), 0, newItem.getTextPath(), 0)
                    markersHelpers.transferMarkers(tx, node.getTextPath(), 0, newItem.getTextPath(), 0)
                    tx.update([container.id, 'nodes'], {type: 'delete', pos: innerNodePos})
                    newListItems.push(newItem.id)
                }
            })

            const newList = tx.create({
                type: 'list',
                items: newListItems,
    ...
    */
    _insertText(tx, sel, text) {
        ...
        markersHelpers.insertText(tx, path, startOffset, endOffset, typeover, L)
    ...
    _breakTextNode(tx, node, coor, container) {
        ...
        // otherwise split the text property and create a new paragraph node with trailing text and annotations transferred
            ...
            if (offset < text.length) {
                ...
                // transfer markers which are after offset to the new node
                markersHelpers.transferMarkers(tx, path, offset, newNode.getTextPath(), 0)
    ...
    _breakListNode(tx, node, coor, container) {
        ...
        // otherwise split the text property and create a new paragraph node with trailing text and annotations transferred
            ...
            if (offset < text.length) {
                ...
                markersHelpers.transferMarkers(tx, path, offset, [newItem.id, 'content'], 0)
    ...
    _mergeNodes(tx, container, pos, direction) {
        const first = container.getChildAt(pos)
        const second = container.getChildAt(pos + 1)

        // When deleting an empty paragraph and there is a list before and after we need it to merge the lists.
        const third = container.hasChildAt(pos + 2)
            ? container.getChildAt(pos + 2)
            : null
        ...
        if (first.isText()) {
            ...
            if (second.isText()) {
                ...
                // transfer markers
                markersHelpers.transferMarkers(tx, sourcePath, 0, targetPath, targetLength)
                tx.delete(source.id)
                tx.setSelection({
                    type: 'property',
                    path: targetPath,
            ...
            // If there is a list both before and after merging them.
            if (third && third.isList()) {
                const thirdItems = third.items.slice()
                const firstItems = first.items.slice()
                const firstPosLength = firstItems.length - 1

                thirdItems.forEach(item => {
                    first.appendItem(item)
                    third.removeItemAt(0)
                })

                container.hide(third.id)
                tx.delete(third.id)

                container.hide(second.id)
                tx.delete(second.id)

                const textPath = tx.get(firstItems[firstPosLength]).getTextPath()
                const text = tx.get(firstItems[firstPosLength]).getText()
