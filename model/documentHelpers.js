...
import markersHelpers from './markersHelpers'
...
function getTextForSelection(doc, sel) {
    if (!sel || sel.isNull()) {
        ...
        for (let i = 0; i < L; i++) {
            ...
            let node = doc.get(id)
            if (node.isList()) {
                const selectedListItems = getListSelectionAsText(doc, sel, node)
...
function getListSelectionAsText(doc, sel, list) {
    const selectionStart = sel.start.path[0]
    const selectionEnd = sel.end.path[0]
    const selectedNodes = (sel.getNodeId ? [sel.getNodeId()] : sel.getNodeIds())

    const selectionBefore = selectedNodes
        .findIndex(itemId => itemId === list.id) > 0

    const selectionAfter = selectedNodes
        .findIndex(itemId => itemId === list.id) < selectedNodes.length - 1

    const startPos = selectionBefore
        ? 0
        : list.getItemPosition(selectionStart)

    const endPos = selectionAfter
        ? list.getLength()
        : list.getItemPosition(selectionEnd) + 1

    const selectionInListNodes = list.items
        .slice(startPos, endPos)
        .map(itemId => {
            const node = doc.get(itemId)
            const text = node.getText()

            if (node.id === sel.start.path[0]) {
                return text.substring(sel.start.offset, text.length)
            }
            else if (node.id === sel.end.path[0]) {
                return text.substring(0, sel.end.offset)
            }
            else {
                return text
            }
...
function deleteListRange(doc, list, start, end,
    ...
    if (firstEntirelySelected) {
        // Remove text from list-item and make it empty.
        if (preserveEmptyListItem) {
            const target = list.getItemAt(startPos)
            const textLength = target.getText().length
            const targetPath = target.getTextPath()
            doc.update(targetPath, {type: 'delete', start: 0, end: textLength})
        }
        else {
...
function mergeListItems(doc, listId, itemPos) {
    ...
    markersHelpers.transferMarkers(doc, sourcePath, 0, targetPath, targetLength)
