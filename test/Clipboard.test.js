...
function ClipboardTests() {
    ...
    test("Copying HTML, and plain text", function(t) {
        ...
        t.ok(clipboardData.data['text/plain'], "Clipboard should contain plain text data.")
        t.ok(clipboardData.data['text/html'], "Clipboard should contain HTML data.")
        let htmlDoc = DefaultDOMElement.parseHTML(clipboardData.data['text/html'])
        let body = htmlDoc.find('body')
        t.ok(body, 'The copied HTML should always be a full HTML document string, containing a body element.')
        t.end()
    })
    ...
    test("Copy and Pasting source code.", function(t) {
        ...
        let cb1 = doc.get('cb1')
        t.notOk(cb1, "Codeblock should have been cutted.")
        clipboard.onPaste(event)
        cb1 = doc.get('cb1')
        t.ok(cb1, "Codeblock should have been pasted.")
