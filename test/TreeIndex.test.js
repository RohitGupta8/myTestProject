...
test("Getting with invalid arguments", function(t) {
    ...
    adapter.set(['a'], 1)
    adapter.set(['c', 'b'], 2)
    t.notOk(adapter.get('e'), 'Should return no value for unknown id')
    t.notOk(adapter.get(['c', 'd']), 'Should return no value for unknown path')
    t.notOk(adapter.get(null), 'Should return no value for null')
    t.notOk(adapter.get(), 'Should return no value for no path')
    t.notOk(adapter.get([]), 'Should return no value for empty path')
    t.notOk(adapter.get({}), 'Should return no value for object')
    t.notOk(adapter.get(1), 'Should return no value for a number')
    t.end()
})
...
test("Arrays: basic usage", function(t) {
    ...
    adapter.delete(['a', 'b'])
    t.notOk(adapter.get(['a', 'b']), 'Value should now be deleted.')
    t.end()
})
