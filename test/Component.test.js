...
function ComponentTests(debug) {
    ...
    const test = module('Component' + (debug ? ' [debug]' : ''), {
        before: function(t) {
    ...
    test("Render an element with custom html", function(t) {
        ...
        t.not(b, null, 'Element should have rendered HTML as content.')
    ...
    test("Dependency-Injection", function(t) {
        ...
        let b = comp.refs.b
        let c = comp.refs.c
        t.not(a.context.foo, undefined, "'a' should have a property 'foo' in its context")
        t.is(a.context.bar, undefined, ".. but not 'bar'")
        t.not(b.context.foo, undefined, "'b' should have a property 'foo' in its context")
        t.is(b.context.bar, undefined, ".. but not 'bar'")
        t.not(c.context.foo, undefined, "'c' should have a property 'foo' in its context")
        t.not(c.context.bar, undefined, ".. and also 'bar'")
        t.end()
    })
    ...
    test("Special nesting situation", function(t) {
        ...
        t.not(foo, null, "Component should have a ref 'foo'.")
        t.equal(foo.el.textContent, 'foo', "foo should have textContent 'foo'")
        t.not(bar, null, "Component should have a ref 'bar'.")
    ...
    test("Special nesting situation II", function(t) {
        ...
        t.not(child, null, "Child should be referenced.")
        t.not(grandchild, null, "Grandchild should be referenced.")
        comp.rerender()
    ...
    test("Implicit retaining should not override higher-level rules", function(t) {
        ...
        t.not(child, null, "Child should exist.")
    ...
    test("Render a child element with ref", function(t) {
        ...
        t.not(comp.refs.foo, null, 'Component should have a ref "foo".')
    ...
    test("didUpdate() provides old props and old state", function(t) {
        ...
        comp.setState({val: 2})
        t.not(oldProps, null, 'old props should have been provided')
        t.equal(oldProps.val, 'a', 'old props should contain original value')
        t.not(oldState, null, 'old state should have been provided')
    ...
    test("Refs on grandchild elements.", function(t) {
        ...
        t.not(comp.refs.foo, null, "Ref 'foo' should be set.")
        let foo = comp.refs.foo
        comp.rerender()
        ...
        t.not(comp.refs.bar, null, "Ref 'bar' should be set.")
    ...
    test("Ref on grandchild component.", function(t) {
        ...
        t.not(comp.refs.grandchild, null, "Ref 'grandchild' should be set.")
        let grandchild = comp.refs.grandchild
        comp.rerender()
        t.not(comp.refs.grandchild, null, "Ref 'grandchild' should be set.")
    ...
    test("#312: refs should be bound to the owner, not to the parent.", function(t) {
        ...
        let comp = Parent.render()
        t.not(comp.refs.foo, null, 'Ref should be bound to owner.')
    ...
    test("Pass-through props and add children via append", function(t) {
        ...
        t.not(simple, null, 'Should have a child component.')
