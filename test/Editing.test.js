...
test("IT10: Typing over an InlineNode node", (t) => {
    ...
    let p1 = doc.get('p1')
    let il1 = doc.get('il1')
    t.notOk(il1, 'InlineNode should have been deleted.')
    t.equal(p1.getText(), P1_TEXT.slice(0, 3) + 'Y' + P1_TEXT.slice(3), 'Text should have been inserted correctly.') // eslint-disable-line no-useless-concat
    t.end()
})
...
test("IB2: Inserting BlockNode using cursor at start of a TextNode", (t) => {
    ...
    t.ok(sel.isNodeSelection(), 'Selection should be on inserted block node')
    t.end()
})
...
test("DL4-1: Deleting using BACKSPACE with cursor inside an empty TextNode and TextNode as predecessor", (t) => {
    ...
    let sel = editorSession.getSelection()
    let p1 = doc.get('p1')
    t.notOk(doc.get('empty'), 'empty node should have been deleted')
...
test("DL5-1: Deleting using BACKSPACE with cursor at the start of a non-empty TextNode and TextNode as predecessor", (t) => {
    ...
    let sel = editorSession.getSelection()
    let p1 = doc.get('p1')
    t.notOk(doc.get('p2'), 'p2 should have been deleted')
...
test("DL13: Deleting using BACKSPACE with cursor after IsolatedNode", (t) => {
    ...
    t.notOk(doc.get('block1'), 'IsolatedNode should have been deleted')
    t.equal(body.getLength(), 1, 'There should be 1 node left')
...
test("D20-9: Deleting a full range across two ListItems within the same List", (t) => {
    const {editorSession, doc} = setupEditor(t, _l1)
    editorSession.setSelection({
        type: 'container',
        startPath: ['l1-1', 'content'],
        startOffset: 0,
    ...
    t.equal(doc.get('l1-1').getText(), '', 'First item to be empty')
    t.notOk(doc.get('l1-2'), 'To not exist')
...
test("C-1 Copying a property selection", (t) => {
    ...
    t.ok(textNode, 'There should be a text node for the property fragment.')
...
test("C-3 Copying a container selection", (t) => {
    ...
    t.ok(content, 'There should be a container node with id "content".')
...
test("C-8 Copying a sparse table", (t) => {
    ...
    for (let i = 0; i < 2; i++) {
        ...
        t.ok(cell, `Cell (${i + 1},${i + 1}) should exist`)
        t.equals(cell.content, T_CONTENT[i][i], '.. and have correct content')
        t.notOk(t1.getCellAt(1 - i, i), `Cell (${1 - i + 1},${i + 1}) should not exist`)
    }
    t.end()
})
