...
test("Mapping a cursor inside a TextProperty from DOM to model", function(t) {
...
test("Mapping a cursor in an empty paragraph from DOM to model", function(t) {
...
test("DOM selection that starts in a TextNode and ends in a paragraph on element level", function(t) {
    let {editor, surface} = setupEditor(t, surfaceWithParagraphs)
    let domSelection = surface.context.domSelection
    let p1Text = editor.el.find('[data-path="p1.content"]').getFirstChild()
...
test("Rendering a ContainerSelection", function(t) {
...
test("Rendering a cursor after inline node", function(t) {
...
test("Rendering a cursor after inline node at the end of a property", function(t) {
    let {editor, doc, surface} = setupEditor(t, paragraphWithInlineNodes)
    let domSelection = surface.context.domSelection
    let sel = doc.createSelection({
        type: 'property',
